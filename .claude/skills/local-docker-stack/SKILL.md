...
- **Storybook preview fails / MSW not found / “Failed to fetch … preview.tsx” in Docker**: follow [`storybook-docker-playwright`](../storybook-docker-playwright/SKILL.md) — refresh `storybook_*` volumes and rebuild the `storybook` image; run `npm run test:storybook` from the repo root.
